...
def index():
    ...
    ## Bar Plot
    ## Lengkapi tahap agregasi untuk membuat dataframe yang mengelompokkan Install berdasarkan Genres
    ## Buatlah bar plot dimana axis x adalah nama Genres dan axis y adalah jumlah aplikasi pada setiap kategori, kemudian urutkan dari jumlah terbanyak
    genres_install = df2.groupby('Genres').agg({
    'Installs' : 'sum'
        }).rename({'Genres':'Installs'}, axis=1).sort_values(by='Installs', ascending=False).head()
    ...
    plt.title('Top 5 Genres Installed')
    # bagian ini digunakan untuk menyimpan plot dalam format image.png
    plt.savefig('genres_install.png',bbox_inches="tight") 
